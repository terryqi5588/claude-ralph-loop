...
╚═══════════════════════════════════════════════════════════════════════╝
                ...
                NO                  YES
                 ...
                 ▼                   ▼
          ┌─────────────┐     ┌─────────────┐
          │ 4. REFLECT  │     │ 5. COMPLETE │
          │             │     │             │
          │ • Record    │     │ • mark done │
          │   reflection│     │ • git commit│
          │ • Check     │     │ • archive   │
          │   attempts  │     └─────────────┘
          └──────┬──────┘            │
                 │                   ▼
    ...
    attempts < 2      attempts ≥ 2
        ...
        │         ┌───────┴────────┬──────────────┐
        │         │                │              │
        │     Decompose        Ask Help     Reset Context
        │         │                │              │
        │         ▼                ▼              ▼
        │    (Split into      [BLOCKER]      (/clear
        │     sub-tasks)       Report         restart)
        │         │
        │         └─────────┐
        │                   │
        ▼                   ▼
        ...
        └──────────────│     < 3?    │
                    ...
                    ┌──────────┴──────────┐
                    │                     │
               Simple fix          Decomposed task
                    │                     │
                    ▼                     ▼
               Back to ACT          Back to PLAN
                    │                     │
                    └──────────┬──────────┘
                               │
                               └──► (Loop continues...)
...
Key Safeguards:
═══════════════
⚠️  Attempts counter: Increments on each 'start', visible in 'python plan.py list'
⚠️  2-failure rule: After 2 failed attempts, MUST switch strategy (no 3rd identical try)
⚠️  3-attempt limit: Task automatically blocked, requires manual intervention
⚠️  Closed loops: Minor fixes → ACT, Strategy changes → PLAN
⚠️  High-signal feedback: Detailed error messages guide effective fixes
```
